...
export default async function QualityPage() {
  ...
  const calibration = equipment.filter((e) => e.calLast && e.calInterval).map((e) => {
    ...
    return { id: e.id, name: e.name, calVendor: e.calVendor, calCert: e.calCert, dueLabel, dueDate: due.toISOString() }
  })
  ...
  return (
    <QualityClient
      ...
      auditEntries={auditEntries.map((a) => ({ id: a.id, entity: a.entity, actor: a.actor, action: a.action, note: a.note, createdAt: a.createdAt.toISOString() }))}
    />
  )
}
