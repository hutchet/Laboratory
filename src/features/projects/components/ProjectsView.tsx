...
export function ProjectsView({ projects, customers, centers }:{ projects:ProjectRow[]; customers:Option[]; centers:Option[] }) {
  ...
  const trends=useMemo(()=>({active:computeSimpleTrend(projects,p=>true,p=>p.createdAt),doing:computeSimpleTrend(projects,p=>p.derivedStatus==="doing",p=>p.createdAt),done:computeSimpleTrend(projects,p=>p.derivedStatus==="done",p=>p.createdAt),risk:computeSimpleTrend(projects,p=>!!p.risk,p=>p.createdAt)}),[projects])
  ...
  return (
    <PageShell title="Dự án">
      ...
      <div className="pager" id="proj-pager" style={{position:"sticky",bottom:0,background:"var(--bg,#f3f4f6)",zIndex:5,borderTop:"2px solid var(--line,#e4e8f0)",marginTop:"auto",...(pageCount<=1?{justifyContent:"flex-start",width:"fit-content"}:{})}}>
