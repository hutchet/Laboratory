  ...
  return (
    <PageShell title="Khách hàng" actions={<Perm minPerm="manager"><button type="button" onClick={()=>{setEditing(null);setShowForm(true)}} style={{padding:"8px 18px",borderRadius:10,border:"none",background:"#1d5fd6",color:"#fff",fontWeight:600,fontSize:13.5,cursor:"pointer"}}>+ Khách hàng mới</button></Perm>}>
      ...
      <div className="section-head">
        ...
        <div className="tools">
          <div style={{position:"relative"}}><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Tìm khách hàng..." style={{padding:"7px 12px 7px 32px",borderRadius:10,border:"1px solid #dfe3e8",fontSize:13,width:220}} /><span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"#9aa1ab"}}>🔍</span></div>
        </div>
      </div>
